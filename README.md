...
### 1.5. 通用调研 (General Research)

**目录**：`evyd-general-research/`

与竞品调研互补的**话题型**桌面调研 skill——用于政策/监管、市场数据、学术综述、趋势分析。**非竞品**，竞品走 `evyd-competitor-research`。

- **三轮方法**：scoping scan（20% 努力，画信息地图）→ targeted deep dive（50%，逐子问题取证）→ cross-validation（30%，交叉核验 + 矛盾暴露）
- **输出 schema**：markdown brief + 机读 JSON 块，claim 带 `[tier · date]` 标签，可信度 high/medium/low。JSON 直接喂 `evyd-ppt-generator` 等下游 skill，不用二次翻译
- **5 级信源分级**：按领域细分（Healthcare SG/SEA、Regulation/AI、Fintech、市场数据、学术技术、中文源），每级给实例。硬规则：高 tier 源存在时不许用低 tier 源
- **工具优先级**：Exa MCP（语义搜索）→ Firecrawl/Jina MCP（页面提取）→ WebSearch → WebFetch（兜底）。Exa + Firecrawl MCP tokens 暂未配，配置模板在 `.mcp.json.template`
- **硬规则**：每条 claim 带源；high confidence 需 ≥2 独立源；矛盾 surface 不许藏；找不到就说 not found；不伪造数字

**触发词**：`帮我查一下`、`做个调研`、`research`、`desk research`、`find data on`（名词是话题/指标而非公司名时走这条）

**输出**：`/tmp/<topic-slug>-brief.md`（markdown + 尾部 JSON fence）

**核心文件**：
- `SKILL.md` — 触发词、工作流、输出 schema、与竞品调研的消歧规则
- `references/source-tiers.md` — 5 级信源分级（按领域，含中文源）
- `references/search-playbook.md` — 三轮 query 模板 + 停止条件 + 常见失败模式
- `references/brief-schema.md` — 输出格式完整 schema（markdown + JSON 字段）
- `references/mcp-setup.md` — Exa / Firecrawl / Jina MCP 配置指南
...
通过本地 Markdown 文件管理团队 TODO，支持布置任务、查看待办、自然语言更新进度、全员汇总、每日定时推送。无外部 API 依赖（不使用企微待办/智能表格），数据存储在 `workspace/todo/team-todo.md`。完成的 TODO 直接删除（不保留已完成标记），报告中的优先级标记（P0/P1/P2）根据 Due 日期动态计算。每日推送通过 OpenClaw cron 实现，渠道无关（企微/飞书/Telegram/webchat 等）。

**适用场景**：团队任务分配与追踪、每日待办查看、进度汇报、全员状态汇总、定时推送待办提醒

**触发词**：`加个todo`、`我的待办`、`XX做完了`、`汇总`、`团队进度`、`给XX布置任务`、`给XX开推送`、`推送状态`

**权限模型**：
- `admin`：创建/查看/更新/删除全员 TODO，查看全员汇总，管理推送
- `member`：查看/更新仅自己的 TODO

**核心文件**：
- `SKILL.md` — 主流程与五个核心操作（各含严格输出模板 + 禁止清单）
- `references/data-format.md` — 数据格式规范（Due 用 YYYY-MM-DD）
- `references/report-format.md` — 汇总报告格式（严格模板 + 动态标记规则 + 禁止清单）
- `references/daily-push.md` — 每日推送配置模板（cron + prompt + 渠道解耦）
...
│   ├── SKILL.md
│   └── references/
├── evyd-general-research/          # 话题型通用调研（政策/市场/学术），输出 brief + JSON
│   ├── SKILL.md
│   └── references/                 # source-tiers / search-playbook / brief-schema / mcp-setup
